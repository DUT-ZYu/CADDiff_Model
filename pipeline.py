...
logger = logging.get_logger(__name__)
 ...
 # 定义抑制低频信息的函数（应用阈值处理）
def suppress_low_frequency_wavelet(tensor, wavelet='haar', factor=1):
    batch_size, channels, height, width = tensor.size()
    tensor_np = tensor.cpu().numpy()
    processed_tensor_np = np.zeros_like(tensor_np)

    for i in range(batch_size):
        for j in range(channels):
            coeffs = pywt.wavedec2(tensor_np[i, j, :, :], wavelet, mode='symmetric', level=1)
            coeffs[0] = coeffs[0] * factor  # 对低频分量应用阈值处理
            processed_tensor_np[i, j, :, :] = pywt.waverec2(coeffs, wavelet, mode='symmetric')

    processed_tensor = torch.tensor(processed_tensor_np).to(tensor.device)
    return processed_tensor


# 定义抑制高频信息的函数（应用阈值处理）
def suppress_high_frequency_wavelet(tensor, wavelet='haar', factor=1):
    batch_size, channels, height, width = tensor.size()
    tensor_np = tensor.cpu().numpy()
    processed_tensor_np = np.zeros_like(tensor_np)
    ...
    for i in range(batch_size):
        for j in range(channels):
            ...
            for k in range(1, len(coeffs)):
                coeffs[k] = tuple([subband * factor for subband in coeffs[k]])  # 对高频分量应用阈值处理
            processed_tensor_np[i, j, :, :] = pywt.waverec2(coeffs, wavelet, mode='symmetric')
...
def adain(cnt_feat, sty_feat):
    cnt_mean = cnt_feat.mean(dim=[1, 2, 3], keepdim=True)
    cnt_std = cnt_feat.std(dim=[ 1, 2, 3], keepdim=True)
    sty_mean = sty_feat.mean(dim=[1, 2, 3], keepdim=True)
    sty_std = sty_feat.std(dim=[1, 2, 3], keepdim=True)
    # output = (cnt_feat - cnt_mean)* sty_std + cnt_mean
    output = (cnt_feat - cnt_mean) * 1.67 + sty_mean
    return output
...
class ZePoPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __init__(
        ...
        if hasattr(scheduler.config, "steps_offset") and scheduler.config.steps_offset != 1:
            deprecation_message = (
                ...
                " it would be very nice if you could open a Pull request for the `scheduler/scheduler_config.json.json`"
                " file"
            )
    ...
    def prepare_latents_con(self, image, timestep, device, dtype, denoise_model, generator=None):
        ...
        if image.shape[1] == 4:
            ...
            if isinstance(generator, list):
                ...
                # init_latents = suppress_high_frequency_wavelet(init_latents)
    ...
    def prepare_latents_sty(self, image, timestep, device, dtype, denoise_model, generator=None):
        image = image.to(device=device, dtype=dtype)
        batch_size = image.shape[0]
        if image.shape[1] == 4:
            init_latents = image
            ...
            if isinstance(generator, list):
                ...
                init_latents = self.vae.encode(image).latent_dist.sample(generator)
                # init_latents = suppress_low_frequency_wavelet(init_latents)
                print('aaaa')
    ...
    def __call__(
            ...
            de_bug=False,
            style_mask=None, source_mask_h=None, source_mask_f=None
    ):
        # 1. Check inputs
        ...
        # spilit f and b
        # print(mutual_latents_b)
        # mutual_latents = mutual_latents_b + mutual_latents * spatial_mask_source

        src_latents, src_clean_latents = self.prepare_latents_con(
            image, latent_timestep, device, dtype, denoise_model, generator
        )
        sty_latents, sty_clean_latents = self.prepare_latents_sty(
            style, latent_timestep, device, dtype, denoise_model, generator
        )
        mutual_latents, _ = self.prepare_latents_con(
            image, timesteps[:1], device, dtype, denoise_model, generator
        )
        mutual_latents = sty_latents + (mutual_latents - sty_latents) * 0.8
        # #==================================================================
        # heigh, width = style_mask.shape[-2:]
        # mask_style = style_mask  # (H, W)
        # mask_source_h = source_mask_h  # (H, W)
        # mask_source_f = source_mask_f  # (H, W)
        # scale = 8
        ...
        # spatial_mask_source = (spatial_mask_source_h + spatial_mask_source_f) / 2
        #
        # mutual_latents_b = mutual_latents * (1- spatial_mask_source)
        # sty_latents_f = sty_latents * spatial_mask_style
        # sty_latents_b = sty_latents * (1-spatial_mask_style)
        # mutual_latents_b = adain(mutual_latents_b, sty_latents_b)
        # # print(mutual_latents_b)
        # mutual_latents = mutual_latents_b + mutual_latents * spatial_mask_source
        # mutual_latents = adain(mutual_latents_b, sty_latents_b) *(1-(spatial_mask_source_h + spatial_mask_source_f)) * 0.1 + \
        #                  mutual_latents * (spatial_mask_source_h + spatial_mask_source_f) * 0.9
        # mutual_latents = mutual_latents * (1- (spatial_mask_source_h + spatial_mask_source_f)) + mutual_latents
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if isinstance(self.scheduler, LCMScheduler):
                    mutual_latents, pred_x0_mutual = self.scheduler.step(mutual_noise_pred, t, mutual_latents,
                                                                         return_dict=False)
                else:
