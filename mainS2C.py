...
def main():
    ...
    for style_dir in os.listdir(args.style):
        ...
        for content_dir in os.listdir(args.content):
            ...
            ptp_utils.register_attention_control(model, controller, tome,
                                    sx=tome_sx,
                                    sy=tome_sy,
                                    ratio=tome_ratio,
                                    de_bug=de_bug,)
            with torch.no_grad():
                    ...
                    save_image(generate_image[-1:], os.path.join(out_dir, f"{content_name}_{style_name}.jpg"), nrow=1, padding=0)
